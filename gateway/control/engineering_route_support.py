...
def handle_system_get_route(req, p: ParseResult, *, db_acquire, db_release) -> bool:
    if p.path == "/health":
        _send_json(
            req,
            200,
    ...
    if p.path == "/metrics":
        ...
        try:
            ...
            metrics["runs_active"] = con.execute(
                "SELECT COUNT(*) as c FROM replay_runs WHERE status IN ('running','paused','queued')"
            ).fetchone()["c"]
            ...
            metrics["journeys_total"] = con.execute("SELECT COUNT(*) as c FROM journeys").fetchone()["c"]
            metrics["screens_total"] = con.execute("SELECT COUNT(*) as c FROM screens").fetchone()["c"]
            metrics["datasets_total"] = con.execute(
                "SELECT COUNT(*) as c FROM synthetic_datasets"
            ).fetchone()["c"]
            db_release(con)
        except Exception:
            pass
        _send_json(req, 200, metrics)
        return True

...
def read_gateway_sessions(
    ...
    *,
    ...
    logname_f = str(logname or "").strip().lower()
    sid_f = str(session_id or "").strip().lower()
    type_f = str(event_type or "").strip().lower()
    q_f = str(q or "").strip().lower()
    uid_f = str(uid or "").strip().lower()
    gid_f = str(gid or "").strip().lower()
    ts_from_i = int(ts_from or 0)
    ts_to_i = int(ts_to or 0)
    ...
    for item in all_sessions:
        ...
        if actor_f and actor_f not in str(item.get("actor") or "").lower():
            matches = False
        if logname_f and logname_f not in str(item.get("logname") or "").lower():
            matches = False
        ...
        if q_f and q_f not in haystack:
            matches = False
        if uid_f and uid_f != str(item.get("uid") or ""):
            matches = False
        if gid_f and gid_f != str(item.get("gid") or ""):
            matches = False
        if ts_from_i > 0 and int(item.get("started_at_ms") or 0) < ts_from_i:
            matches = False
        if ts_to_i > 0 and int(item.get("last_ts_ms") or 0) > ts_to_i:
            matches = False
        item["matched"] = matches
    matched = [item for item in all_sessions if item.get("matched") or (not actor_f and not logname_f and not sid_f and not type_f and not q_f and not uid_f and not gid_f and ts_from_i <= 0 and ts_to_i <= 0)]

...
function renderScenarios(items) {
  ...
  // Visibilidade exibida em pt-BR; o valor da API (private/shared) não muda.
  const visibilityLabel = (v) => (String(v || "private") === "shared" ? "compartilhada" : "privada");
  html(
    "#obs_scenarios",
    (items || []).length
          ...
          <div class="r2ctl-obs-run">
            <div class="flex items-center justify-between gap-3">
              <div>
                ...
                <div class="mt-1 text-[11px] text-stone-400">${escapeHtml(visibilityLabel(item.visibility))} • etiquetas: ${escapeHtml((item.tags || []).join(",") || "-")}</div>
              </div>
...
async function loadOverview() {
  ...
  // Rótulos leigos dos filtros ativos; a chave original da API fica entre parênteses.
  const FILTER_LABELS = {
    log_dir: "diretório de log (log_dir)",
    environment: "ambiente",
    created_from_ms: "criado a partir de, ms (created_from_ms)",
    created_to_ms: "criado até, ms (created_to_ms)",
    run_limit: "limite de execuções (run_limit)",
  };
  html("#obs_active_filters", Object.entries((trend.summary || {}).filters || currentScenarioFilters()).map(([key, value]) => `<div class="flex items-center justify-between rounded-xl border border-stone-800 bg-stone-950/40 px-3 py-2"><span class="text-xs uppercase tracking-[0.14em] text-stone-400">${escapeHtml(FILTER_LABELS[key] || key)}</span><span class="font-mono text-xs text-stone-200">${escapeHtml(value || "-")}</span></div>`).join(""));
  ...
  html("#obs_reprocess_signatures", textListCards(reprocess.repeated_signatures || [], "Sem assinaturas repetidas.", (item) => `repetição: ${escapeHtml(item.repeat_rate_pct || 0)}%`));
  ...
  html("#obs_reprocess_candidates", textListCards(reprocess.automation_candidates || [], "Sem candidatas fortes a automação.", (item) => `pontuação: ${escapeHtml(item.automation_candidate_score || 0)}`));
  ...
  html("#obs_trend_environments", textListCards(trend.environments || [], "Sem tendência por ambiente.", (item) => `execuções: ${formatCount(item.runs || 0)} • regressões: ${formatCount(item.regressions || 0)}`));

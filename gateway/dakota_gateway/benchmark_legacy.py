"""Compatibilidade legada do endpoint antigo ``/api/synthetic/benchmark``.

ATENÇÃO: este módulo é uma SIMULAÇÃO de benchmark (números gerados por
``random.Random`` a partir do seed). Ele NÃO executa nada contra servidores
reais e NUNCA deve ser usado para decisão de migração. O benchmark oficial —
replay real, pareado, com contrato imutável — vive no pacote
``dakota_gateway.benchmark`` (ver ``dev/benchmark-api-contract.md``).

O módulo existe apenas para manter funcionando o endpoint antigo
``POST /api/synthetic/benchmark`` e o subcomando legado
``dakota-gateway synthetic benchmark``, que respondem resultados marcados
como simulação. Fica fora do pacote ``benchmark/`` justamente porque a
varredura AST de ``tests/benchmark/test_no_simulation_official.py`` proíbe
qualquer geração aleatória/simulada dentro do pacote oficial.
"""
...
class BenchmarkConfig:
    """Configuracao de benchmark (legado — simulação)."""
    benchmark_id: str = ""
    name: str = ""
    journey_id: str = ""
    dataset_name: str = ""
...
class EnvironmentMetrics:
    """Metricas coletadas de um ambiente (legado — simulação)."""
    environment: str = ""
    host: str = ""
...
class BenchmarkComparison:
    """Comparacao entre dois ambientes (legado — simulação)."""
    baseline_env: str = ""
    target_env: str = ""
...
class BenchmarkResult:
    """Resultado completo do benchmark (legado — simulação)."""
    benchmark_id: str = ""
    name: str = ""
...
class BenchmarkOrchestrator:
    """Orquestrador legado de benchmark — SIMULAÇÃO, não usar p/ migração."""

    def __init__(self, db_path: str = ""):
        self.db_path = db_path

    def run(self, config: BenchmarkConfig) -> BenchmarkResult:
        """Executa benchmark completo (SIMULAÇÃO determinística por seed)."""
        start = time.time()
    ...
    def _run_single(
        ...
        """Executa uma iteracao em um ambiente (SIMULAÇÃO — sem servidor real)."""
        metrics = EnvironmentMetrics(
            environment=env.get("name", ""),
            host=env.get("host", ""),
        )

        # SIMULAÇÃO legada: números derivados do seed, sem replay real.
        # Não confundir com dakota_gateway.benchmark (oficial, replay real).
        import random
    ...
    def run_and_report(self, config: BenchmarkConfig) -> dict:
        """Executa e retorna relatorio em dicionario (marcado como simulação).

        Este orquestrador é SIMULAÇÃO determinística por seed: os números não
        são medição real. Por isso o relatório NUNCA inclui recomendação de
        migração (números sintéticos não sustentam decisão) e sempre carrega
        ``simulation=True`` + ``simulation_notice``. O caminho oficial é o
        benchmark real (dakota_gateway/benchmark/, §21 da spec).
        """
        ...
        return {
            ...
            "simulation_notice": (
                ...
                "(CLI 'benchmark' / POST /api/benchmarks)."
            ),
            ...
            "comparisons": [
                {
                    ...
                    "verdict": c.verdict,
                    "metrics": c.metrics,
                    # sem "recommendations": simulacao nao recomenda migracao
                }
                for c in result.comparisons
            ],

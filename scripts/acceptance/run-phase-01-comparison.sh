...
run_step phase01-structural 30 bash -c '! rg -n "if not has_canonical and mode == .visual.|mode = .hybrid.  # fallback automatico|got == expected_sig|signature_now\\(\\).*==|== expected_sig" gateway/dakota_gateway/replay.py gateway/dakota_gateway/replay_control/'

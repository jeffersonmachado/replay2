...
run_step python-full 900 env -u DAKOTA_PROCESS_RUN_ID python -m pytest -q tests/
run_step gateway-tests "$ACCEPTANCE_TIMEOUT" env -u DAKOTA_PROCESS_RUN_ID python -m pytest -q gateway/tests/
run_step javascript-tests 60 env -u DAKOTA_PROCESS_RUN_ID node --test \
  gateway/control/static/js/components/terminal_snapshot_renderer.test.mjs \
  gateway/control/static/js/components/replay_snapshot_state.test.mjs
run_step tcl-tests 30 env -u DAKOTA_PROCESS_RUN_ID tclsh tests/all.tcl

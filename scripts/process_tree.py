...
# Regra da cadeia de release (spec §25): NAO existe allowlist por nome de
# comm. QUALQUER processo com DAKOTA_PROCESS_RUN_ID que sobreviva ao pai e'
# classificado como leaked (e escaped quando a sessao difere do lider da run),
# eliminado, com success=false — sem excecao para chrome_crashpad, browsers ou
# helpers. Testes que sobem browsers REAIS devem matar o browser e os filhos
# no proprio teardown (ex.: tests/test_terminal_snapshot_css_contract.py e
# tests/test_web_ui_selenium.py).
...
def run_with_timeout(
    ...
    # Sem allowlist por nome: qualquer sobrevivente com o run_id e' escaped
    # (sessao propria/reparentado) e/ou leaked (vivo apos o exit do pai).
